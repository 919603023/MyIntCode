...
int AnalyzeAgreement(char *buf)
{
	...
	else if (type == 0x0806)
	{
		...
		if (ntohs(*(unsigned short *)(arp + 6)) == 1)mybuf.type = ARP_GO;

		if (ntohs(*(unsigned short *)(arp + 6)) == 2)mybuf.type = ARP_BACK;
	...
	else if (type == 0x8035)
	{
		...
		if (ntohs(*(unsigned short *)(rarp + 6)) == 3)mybuf.type = RARP_GO;

		if (ntohs(*(unsigned short *)(rarp + 6)) == 4)mybuf.type = RARP_BACK;
	}
	else
	{
		mybuf.type = NuLL;
